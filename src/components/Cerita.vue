    ...
    <div>
        <div v-if="username" class="pt-24 flex justify-between max-w-[1400px] m-auto">
            <ul class="max-w-2xl divide-y rounded-xl">
                <li>
                    <details class="group">
                        <summary class="font-inter text-white text-xl flex bg-teal items-center gap-2  mt-16  font-bold hover:cursor-pointer rounded-md flex items-center gap-3 px-4 py-3 font-medium marker:content-none hover:cursor-pointer ">
                        ...
                        <article class="absolute max-h-[300px] overflow-y-scroll max-w-[534px] bg-white mx-4 pb-4">
                            <div v-for="story in updatePaginatedUserStory()" :key="story.id" class="flex justify-center items-center gap-4 px-4 py-4 shadow-md text-opacity-40">
                                <p class="font-inter text-fullblack font-medium text-[14px]">{{ story.title }}</p>
                                <p class="font-inter text-fullblack font-medium text-[14px]" v-if="story.isVerified === true">Sudah di Verifikasi</p>
                                <p class="font-inter text-fullblack font-medium text-[14px]" v-else-if="story.isVerified === false">Pending</p>
                                <p class="font-inter text-fullblack font-medium text-[14px]">{{ formatDate(story.createdAt) }}</p>
                            </div>
                        </article>
                    </details>
                </li>
            </ul>
            <button v-on:click="toggleModal()" class="font-inter text-white text-xl flex bg-teal items-center gap-2 p-2 mt-16 mb-4 font-bold hover:cursor-pointer rounded-md">Unggah Cerita</button>
        </div>
        ...
        <div class="grid grid-cols-2 gap-8 pb-4 px-16 max-w-[1400px] m-auto items-end">
            <div class="flex flex-col" v-for="cerita in updatePaginatedAllStory()" :key="cerita.id">
                ...
                <a :href="'detailblog/' + cerita.id"><button class="rounded-md bg-teal py-4 text-white text-xl font-bold w-full mt-4">Lihat Semua</button></a>
                <div class="flex gap-1 items-center justify-center mt-4">
                    ...
                    <p class="font-poppins font-normal text-[12px] text-darktransparent">{{ formatDate(cerita.createdAt) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="mt-4 flex justify-center">
            <button @click="prevPageAllStory" :disabled="currentPageAllStory === 1" class="px-4 py-2 mr-2 bg-teal text-white rounded-md">Previous</button>
            <button v-for="pageNumber in totalPagesAllStory" :key="pageNumber" @click="goToPageAllStory(pageNumber)"
                ...
                class="px-4 py-2 mr-2">{{ pageNumber }}</button>
            <button @click="nextPageAllStory" :disabled="currentPageAllStory === totalPagesAllStory" class="px-4 py-2 bg-teal text-white rounded-md">Next</button>
        </div>
    </div>
                ...
                <div class="border border-red rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                    ...
                    <div class="flex items-center justify-center p-6 border-t-2 border-black rounded-b">
                        <router-link to="/createcerita">
                            <button class="text-white bg-teal border hover:text-white active:bg-teal-600 font-bold uppercase text-sm px-12 py-3 rounded outline-none focus:outline-none mr-1 mb-1" type="button">Setuju</button>
                        </router-link>
                        <button class="text-teal bg-white border active:bg-teal-600 font-bold uppercase text-sm px-6 py-3 rounded outline-none focus:outline-none mr-1 mb-1" type="button" v-on:click="toggleModal()">Tidak Setuju</button>
                    </div>
                </div>
            </div>
        </div>
